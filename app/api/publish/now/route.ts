...
async function postToFacebook(args: {
  ...
  if (args.imageUrl && args.imageUrl.trim()) {
    ...
    if (!isLikelyImageUrl(imageUrl)) {
      return { ok: false, status: 400, json: { error: { message: "Facebook imageUrl must be a direct https image link (ending .jpg/.png etc)." } } };
    }
...
async function postToLinkedInViaInternal(req: NextRequest, organisationId: string, message: string) {
  // call your existing LinkedIn route (keeps your current implementation intact)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const platforms: ProviderId[] = platformsRaw.map((p: any) => String(p || "").toLowerCase().trim()).filter(Boolean) as ProviderId[];

    if (!id) return NextResponse.json({ success: false, error: "Missing id" }, { status: 400 });
    if (platforms.length === 0) return NextResponse.json({ success: false, error: "Pick at least one platform" }, { status: 400 });

    // 1) Load the scheduled post row
    ...
    // 2) Publish (direct) per selected platforms
    const results: any[] = [];
    ...
    // 3) Persist outcome to scheduled_posts (this is the missing piece)
    ...
    if (upErr) {
      ...
      // still return publish results, but signal persistence issue
