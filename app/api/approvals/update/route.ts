...
export async function POST(req: NextRequest) {
  try {
    ...
    // Pull existing meta so we can safely merge
    ...
    if (readErr) {
      ...
      return NextResponse.json({ success: false, error: "Failed to read post", details: readErr.message }, { status: 500 });
    }
    ...
    return NextResponse.json({ success: true, id: data.id, status: data.status }, { status: 200 });
  } catch (err: any) {
