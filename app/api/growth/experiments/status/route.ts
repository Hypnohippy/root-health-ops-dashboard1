...
export async function POST(req: NextRequest) {
  try {
    ...
    const status = norm(body?.status).toLowerCase() as ExperimentStatus;

    if (!id || !status || !["planned", "running", "completed"].includes(status)) {
      return NextResponse.json({ success: false, error: "Missing/invalid id or status." }, { status: 400 });
    }
